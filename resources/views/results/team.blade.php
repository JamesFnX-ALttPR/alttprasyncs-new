...
@php
$teamresults = $raceresult->result()->select('team', DB::raw('AVG(time) AS avg_time'))->groupBy('team')->orderBy('avg_time', 'ASC')->get();
@endphp
...
<a class="text-blue-300 hover:underline" href="/mode/{{ $raceresult->mode_id }}">coop {{ $mode }}</a>@if($mode_desc != null) - {{ $mode_desc }}@endif<br />
@if($raceresult->spoiler_race == 1)
<br /><a target="_blank" href="{{ $raceresult->spoiler_log }}">Link to Spoiler</a><br />
@endif
        Average Time - {{ date("G:i:s", round($raceresult->result_avg_time)) }}</p><hr />
            ...
            <table class="w-full max-w-3/4 mx-auto border-collapse border border-gray-400 rounded-md">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="{{ $class }}">{{ $team->team }}</td>
@if (DB::table('results')->where('race_id', $id)->where('team', $team->team)->max('forfeit') == 1)
                        <td class="{{ $class }}" colspan="2">Forfeit</td>
@else
                        <td class="{{ $class }}" colspan="2">{{ date("G:i:s", round($team->avg_time)) }}</td>
@endif
                    </tr>
                    @foreach ($raceresult->result()->where('team', $team->team)->get() as $result)
                    <tr>
                        <td class="{{ $subclass }}"></td>
                        <td class="{{ $subclass }}"><a class="text-blue-300 hover:underline" href="/racer/{{ $result->racer->id }}">{{ $result->racer->name }}</a></td>
                        <td class="{{ $subclass }}">@if ($result->forfeit == 1)Forfeit @else{{ date("G:i:s", $result->time) }}@endif</td>
                        <td class="{{ $subclass }}">{{ $result->comment }}</td>
                    </tr>                    
                    @endforeach
                    @endforeach
                </tbody>

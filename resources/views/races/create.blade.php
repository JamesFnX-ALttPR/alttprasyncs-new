<x-layout>
    <x-slot:title>Create New Race</x-slot:title>
    <x-slot:heading>Create New Race</x-slot:heading>
    <div class="justify-center">
        <form class="max-w-2/5 mx-auto" method="POST" action="/races/create">
            @csrf
            
            <div class="mb-5">
                <x-form-label for="mode">Mode</x-form-label>
                <select id="mode" name="mode" class="w-full bg-transparent placeholder:text-slate-400 text-slate-700 text-sm border border-slate-200 rounded pl-3 pr-8 py-2 transition duration-300 ease focus:outline-none focus:border-slate-400 hover:border-slate-400 shadow-sm focus:shadow-md appearance-none cursor-pointer">
                    <option value=""></option>
                    <option value="0">My mode isn't in this list</option>
@foreach ($modes as $mode)
                    <option value="{{ $mode->id }}" @selected(old('mode') == $mode->id)>{{ $mode->name }}</option>    
@endforeach
                </select>
                ...
                <x-form-input id="seed" name="seed" value="{{ old('seed') }}" />
                <x-form-error type="seed" />
                <x-form-label for="description">Description</x-form-label>
                <x-form-input id="description" name="description" value="{{ old('description') }}" />
                ...
                <div class="grid grid-cols-5 content-start justify-items-center">
                    <div>
                        <x-form-hash-select>hash_1</x-form-hash-select>
                    </div>
                    <div>
                        <x-form-hash-select>hash_2</x-form-hash-select>
                    </div>
                    <div>
                        <x-form-hash-select>hash_3</x-form-hash-select>
                    </div>
                    <div>
                        <x-form-hash-select>hash_4</x-form-hash-select>
                    </div>
                    <div>
                        <x-form-hash-select>hash_5</x-form-hash-select>
                    </div>
                </div>

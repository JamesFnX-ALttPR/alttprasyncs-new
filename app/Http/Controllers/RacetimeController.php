...
{
    public static function getModeData(string $str) {
    ...
    public static function gatherALttPRDataURLs(int $int) {
        $url = "https://racetime.gg/alttpr/races/data?page=" . $int;
        $response = Http::get($url);
        $json = $response->json();
        $result_array = [];
        for ($i = 0; $i < count($json['races']); $i++) {
            array_unshift($result_array, 'https://racetime.gg' . $json['races'][$i]['data_url']);
        }
        return $result_array;
    }

    public function getRacetimeData(string $url) {
        $response = Http::get($url);
        $json = $response->json();
        return $json;
    }

    public function parseRacetimeData(array $data) {
        ...
        $parsed_hash = self::validateHash($hash);
    ...
    public function getSpoilerLog($name) {
    ...
    public function validateHash($str) {
        ...
        if (preg_match($sahabot_pattern, $str)) {
            preg_match($sahabot_pattern, $str, $matches);
            for($i=1; $i<6; $i++) {
        ...
        } elseif (preg_match($mudora_pattern, $str)) {
            preg_match($mudora_pattern, $str, $matches);
            for($i= 1; $i< 6; $i++) {
                if (!in_array($matches[$i], $valid_hashes)) {
                    return false;
                }
    ...
    public function getRacerData(array $data, int $ordinal) {
